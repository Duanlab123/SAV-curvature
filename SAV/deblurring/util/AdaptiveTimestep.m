function [ phi,tau_n,Energy_new,afa,du_x,du_y,grad_u,inter_data ] = AdaptiveTimestep( par,afa,du_x,du_y,grad_u,phi,inter_data,f,DtD,tau_n,phi_old,rn,Energy,kernel)
   [phi]=new_order1(par,afa,du_x,du_y,grad_u,phi,inter_data,f,DtD,tau_n,rn,kernel);
   %step1: compute u^{n+1}
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   Energy_new=sum(sum(0.5*par.ep*grad_u.^2))+r_new^2;       
    ...
    %    [phi,tau_n,Energy_new]=AdaptiveTimestep(par,afa,du_x,du_y,grad_u,phi,LR1,DtD,tau_n,phi_old,rn,Energy,upscaling,h,kernel);
       [ phi,tau_n,Energy_new,afa,du_x,du_y,grad_u,inter_data ] = AdaptiveTimestep( par,afa,du_x,du_y,grad_u,phi,inter_data,f,DtD,tau_n,phi_old,rn,Energy,kernel);
    end
end
